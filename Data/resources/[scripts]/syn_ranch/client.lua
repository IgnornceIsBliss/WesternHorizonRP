...
-- Erval
local reachdist = Config.minrange /2
local goat
local reached = false
local goal
    ...
    while true do
       ...
       if registering then 
        ...
        if not reg then 
            ...
            if range > dist1 and dist1 > Config.minrange then
                drawtext(Config.Language.pressg, 0.15, 0.10, 0.1, 0.3, true, 255, 255, 255, 255, true, 10000)
        ...
        if reg and not reg1 then
            ...
            if range > dist2 and dist2 > Config.minrange then
                drawtext(Config.Language.pressg, 0.15, 0.10, 0.1, 0.3, true, 255, 255, 255, 255, true, 10000)
        ...
        if reg and reg1 and not reg2 then 
            ...
            if range > dist3 and dist3 > Config.minrange then
                drawtext(Config.Language.pressg, 0.15, 0.10, 0.1, 0.3, true, 255, 255, 255, 255, true, 10000)
        ...
        if reg and reg1 and reg2 and not reg3 then 
            ...
            if range > dist4 and dist4 > Config.minrange then
                drawtext(Config.Language.pressg, 0.15, 0.10, 0.1, 0.3, true, 255, 255, 255, 255, true, 10000)
...
AddEventHandler("syn_ranch:doOpenMenuWithInfos", function(ranchInfos,activetasks2) -- Cool down, infos cattle, qualité sur le serveur
    playerRanch_Infos = ranchInfos
...
Citizen.CreateThread( function()
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('ranchchores') then
            ...
            if contains(playerRanch_Infos.chores, "racking") and 2 > chores["racking"].num and not contains2(activeTasks, "racking") then 
            ...
            if contains(playerRanch_Infos.chores, "hayscope") and 2 > chores["hayscope"].num and not contains2(activeTasks, "hayscope") then 
            ...
            if contains(playerRanch_Infos.chores, "feed") and 2 > chores["feed"].num and not contains2(activeTasks, "feed")  then 
            ...
            if contains(playerRanch_Infos.chores, "hayspread") and 2 > chores["hayspread"].num and not contains2(activeTasks, "hayspread")  then 
            ...
            if contains(playerRanch_Infos.chores, "water") and 2 > chores["water"].num  and not contains2(activeTasks, "water") then 
       ...
       if not onme and ontheway then
            if first == false then
                for k,v in pairs(herd) do 
                    ...
                    if reachdist > caldist and ontheway then
                        reset = true
       ...
       Citizen.Wait(100)
            for k,v in pairs(herd) do 
                 ...
                 if first == true and sec == true and third == true and fourth == true and fifth and sixth and reset6 then 
                     if reachdist > caldist and ontheway and not reachedtimerout then
                         reached = true
                         DeleteEntity(v.goat)
       ...
       Citizen.Wait(100)
            for k,v in pairs(herd) do 
                 ...
                 for f, g in pairs(Config.cattlesale) do
                     ...
                     if reachdist > caldist and ontheway2 and not reachedtimerout2 then
                         reached2 = true
                         DeleteEntity(v.goat)
                         finalnumber = finalnumber + 1
    ...
    while true do
       ...
       if ontheway and not reached then
            if IsEntityDead(PlayerPedId()) or timeout or failedherding then 
                ...
                if q_cattlex[typeAnimal] and 0 >= q_cattlex[typeAnimal] then 
                    cattlex[typeAnimal] = nil
                    q_cattlex[typeAnimal] = nil
                end
       ...
       if ontheway then 
            for k,v in pairs(herd) do 
                if dog ~= "0" then -- Avec un chien gardien de troupeau
                    ...
                    if caldist2 > Config.minrange and caldist > 15 and ontheway and not v.stray and not onme then -- Troupeau trop loin du joueur : il attend le joueur
                         ClearPedTasks(v.goat)
                         FreezeEntityPosition(v.goat,true)
                         v.far = true
                    ...
                    local xcoords = GetEntityCoords(v.goat)
                        local zcoords = GetEntityCoords(PlayerPedId())
                        local caldist2 = GetDistanceBetweenCoords(zcoords.x, zcoords.y, zcoords.z, xcoords.x, xcoords.y, xcoords.z,0)
                    if caldist2 > Config.minrange and ontheway and not v.stray and not onme then
                         ClearPedTasks(v.goat)
                         FreezeEntityPosition(v.goat,true)
                         v.far = true
            ...
            elseif dist <= 3 then
                ...
                SetNuiFocus(true,true)
                    ...
                    elseif ranchTaskName == "water" then
                        ...
                        TaskStartScenarioInPlace(PlayerPedId(), GetHashKey("WORLD_PLAYER_CHORES_BUCKET_POUR_LOW"), 15000, true, false, false, false)
