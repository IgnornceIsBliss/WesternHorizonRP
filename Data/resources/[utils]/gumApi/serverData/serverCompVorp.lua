if Config.Framework == "VORP" then
    ...
    function apiSrv.setGrade(identifier, charidentifier, jobId, jobGrade)
        exports.oxmysql:execute('UPDATE characters SET jobgrade = ? WHERE identifier = ? AND charidentifier = ?', {jobGrade, identifier, charidentifier})
    end

    function apiSrv.removeJob(identifier, charidentifier, jobId)
        exports.oxmysql:execute('UPDATE characters SET job = ?, jobgrade = ?, joblabel = ?  WHERE identifier = ? AND charidentifier = ?', {"unemployed", 0, "Without Job", identifier, charidentifier})
    end
    ...
    function apiSrv.getInformation(source, cb)
        ...
        if User then
            ...
            player.setJob = function(job, grade, arg)
                ...
                Character.setJobLabel(job)
            end

            player.setGrade = function(amount)
                Character.setJobGrade(amount)
                exports.oxmysql:execute('UPDATE characters SET jobgrade = ? WHERE identifier = ? AND charidentifier = ?', {amount, Character.identifier, Character.charIdentifier})
            end
            ...
            player.getCountItem = function(id)
                if Inventory.getItemCount(tonumber(_source), id) == nil then
                    return 0
                end
            ...
            player.updateStorage = function(id, size)
                Inventory.updateCustomInventorySlots(_source, id, size)
            end
            ...
            player.giveWeapon = function(id, playerId)
                exports.vorp_inventory:createWeapon(_source, id, {}, {}, {}, function(success) end)
            end
